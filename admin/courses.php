    ...
    <div class="content-wrapper">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#courseModal"
                onclick="resetCourseForm()"><i class="fas fa-plus me-1"></i> Add Course</button>
        </div>
        <div class="row g-3">
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="course-card">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            ...
                            <span
                                class="badge <?= $c['status'] === 'active' ? 'bg-success' : 'bg-secondary' ?>"><?= ucfirst($c['status']) ?></span>
                        </div>
                        <div class="course-name"><?= e($c['name']) ?></div>
                        <div class="course-info mb-1"><i class="fas fa-clock me-1"></i> <?= $c['credit_hours'] ?> Credit
                            Hours</div>
                        <div class="course-info mb-1"><i class="fas fa-building me-1"></i> <?= e($c['department']) ?></div>
                        <div class="course-info mb-3"><i class="fas fa-layer-group me-1"></i> <?= $c['section_count'] ?>
                            Active Sections</div>
                        <div class="d-flex gap-2">
                            <button class="btn btn-sm btn-outline-primary flex-fill"
                                onclick='editCourse(<?= json_encode($c) ?>)'><i class="fas fa-edit me-1"></i>Edit</button>
                            <form method="POST" onsubmit="return confirmDelete()"><input type="hidden" name="action"
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="cModalTitle">Add Course</h5><button type="button" class="btn-close"
                    data-bs-dismiss="modal"></button>
            </div>
            <form method="POST">
                <div class="modal-body">
                    <input type="hidden" name="action" id="cAction" value="add"><input type="hidden" name="id" id="cId">
                    <div class="mb-3"><label class="form-label">Course Code</label><input type="text" name="code"
                            id="c_code" class="form-control" required></div>
                    <div class="mb-3"><label class="form-label">Course Name</label><input type="text" name="name"
                            id="c_name" class="form-control" required></div>
                    <div class="row g-3 mb-3">
                        <div class="col-6"><label class="form-label">Credit Hours</label><input type="number"
                                name="credit_hours" id="c_ch" class="form-control" value="3" min="1" max="6"></div>
                        <div class="col-6"><label class="form-label">Status</label><select name="status" id="c_status"
                                class="form-select">
                                <option value="active">Active</option>
                                <option value="inactive">Inactive</option>
                            </select></div>
                    </div>
                    <div class="mb-3"><label class="form-label">Department</label><input type="text" name="department"
                            id="c_dept" class="form-control"></div>
                    <div class="mb-3"><label class="form-label">Description</label><textarea name="description"
                            id="c_desc" class="form-control" rows="3"></textarea></div>
                </div>
