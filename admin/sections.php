<?php
/** MOEEN  - Admin Section Management */
...
<main class="main-content">
    <div class="content-wrapper">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#secModal"><i
                    class="fas fa-plus me-1"></i> Add Section</button>
        </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php foreach ($sections as $s): ?>
                            <tr>
                                <td><span class="fw-semibold"><?= e($s['course_code']) ?></span> -
                                    <?= e($s['course_name']) ?></td>
                                ...
                                <td><?= e($s['room']) ?></td>
                                <td><span class="badge bg-info"><?= $s['student_count'] ?>/<?= $s['max_students'] ?></span>
                                </td>
                                <td><span
                                        class="badge <?= $s['status'] === 'active' ? 'bg-success' : 'bg-secondary' ?>"><?= ucfirst($s['status']) ?></span>
                                </td>
                                <td>
                                    <form method="POST" class="d-inline" onsubmit="return confirmDelete()"><input
                                            ...
                                            value="<?= $s['id'] ?>"><button class="btn btn-sm btn-outline-danger"><i
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        <div class="col-8"><label class="form-label">Course</label><select name="course_id"
                                class="form-select" required><?php foreach ($courses as $c): ?>
                                    <option value="<?= $c['id'] ?>"><?= e($c['code'] . ' - ' . $c['name']) ?></option>
                                <?php endforeach; ?>
                            </select></div>
                        <div class="col-4"><label class="form-label">Section #</label><input type="text"
                                name="section_number" class="form-control" required></div>
                        <div class="col-12"><label class="form-label">Faculty</label><select name="faculty_id"
                                class="form-select" required><?php foreach ($faculty as $f): ?>
                                    <option value="<?= $f['id'] ?>"><?= e($f['name']) ?></option><?php endforeach; ?>
                            </select></div>
                        <div class="col-6"><label class="form-label">Semester</label><input type="text" name="semester"
                                class="form-control" value="<?= CURRENT_SEMESTER ?>" required></div>
                        <div class="col-6"><label class="form-label">Academic Year</label><input type="text"
                                name="academic_year" class="form-control" value="<?= ACADEMIC_YEAR ?>" required></div>
                        <div class="col-4"><label class="form-label">Max Students</label><input type="number"
                                name="max_students" class="form-control" value="35"></div>
                        <div class="col-8"><label class="form-label">Schedule</label><input type="text" name="schedule"
                                class="form-control" placeholder="e.g. Sun-Tue 10:00-11:30"></div>
                        <div class="col-12"><label class="form-label">Room</label><input type="text" name="room"
