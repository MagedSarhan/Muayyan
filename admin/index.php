    ...
    <div class="content-wrapper">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h6><i class="fas fa-shield-alt me-2"></i>Alerts by Severity</h6>
                    </div>
                    <div class="card-body">
                        <div class="chart-wrapper" style="height:250px">
            ...
            <div class="col-lg-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h6><i class="fas fa-tachometer-alt me-2"></i>Quick Overview</h6>
                    </div>
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3 p-2 rounded"
                            style="background:var(--primary-25)">
                            <span class="fw-semibold" style="font-size:.85rem"><i
                                    class="fas fa-user-graduate me-2 text-primary"></i>Students</span>
                            <span class="badge bg-primary"><?= $totalStudents ?></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-3 p-2 rounded"
                            style="background:var(--primary-25)">
                            <span class="fw-semibold" style="font-size:.85rem"><i
                                    class="fas fa-chalkboard-teacher me-2 text-success"></i>Faculty</span>
                            <span class="badge bg-success"><?= $totalFaculty ?></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-3 p-2 rounded"
                            style="background:var(--primary-25)">
                            <span class="fw-semibold" style="font-size:.85rem"><i
                                    class="fas fa-clipboard-list me-2 text-warning"></i>Assessments</span>
                            <span
                                class="badge bg-warning text-dark"><?= $db->query("SELECT COUNT(*) FROM assessments")->fetchColumn() ?></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-3 p-2 rounded"
                            style="background:var(--primary-25)">
                            <span class="fw-semibold" style="font-size:.85rem"><i
                                    class="fas fa-envelope me-2 text-info"></i>Contact Requests</span>
                            <span
                                class="badge bg-info"><?= $db->query("SELECT COUNT(*) FROM contact_requests")->fetchColumn() ?></span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center p-2 rounded"
                            style="background:var(--primary-25)">
                            <span class="fw-semibold" style="font-size:.85rem"><i
                                    class="fas fa-pen me-2 text-danger"></i>Grades Entered</span>
                            <span
                                class="badge bg-danger"><?= $db->query("SELECT COUNT(*) FROM grades")->fetchColumn() ?></span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row g-4 mt-1">
            <div class="col-lg-4">
                ...
                <div class="row g-3">
                    <div class="col-6"><a href="<?= BASE_URL ?>/admin/users.php?action=add" class="quick-action"><i
                                class="fas fa-user-plus"></i><span>Add User</span></a></div>
                    <div class="col-6"><a href="<?= BASE_URL ?>/admin/courses.php" class="quick-action"><i
                                class="fas fa-plus-circle"></i><span>Add Course</span></a></div>
                    <div class="col-6"><a href="<?= BASE_URL ?>/admin/reports.php" class="quick-action"><i
                                class="fas fa-chart-bar"></i><span>View Reports</span></a></div>
                    <div class="col-6"><a href="<?= BASE_URL ?>/admin/settings.php" class="quick-action"><i
                                class="fas fa-cog"></i><span>Settings</span></a></div>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <?php foreach ($recentActivity as $activity): ?>
                            <div class="activity-item">
                                <div
                                    class="activity-dot <?= strpos($activity['action'], 'login') !== false ? 'primary' : (strpos($activity['action'], 'grade') !== false ? 'success' : 'warning') ?>">
                                </div>
                                <div class="activity-content">
                                    <div class="title"><?= e($activity['user_name'] ?? 'System') ?> —
                                        <?= e(ucfirst(str_replace('_', ' ', $activity['action']))) ?></div>
                                    <div class="time"><?= e($activity['description']) ?> &bull;
                                        <?= timeAgo($activity['created_at']) ?></div>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (userRoleCtx) {
            new Chart(userRoleCtx, {
                type: 'doughnut',
                data: {
                    labels: [<?php foreach ($roleStats as $r)
                        echo "'" . ucfirst($r['role']) . "',"; ?>],
                    datasets: [{
                        data: [<?php foreach ($roleStats as $r)
                            echo $r['count'] . ","; ?>],
        ...
        if (alertCtx) {
            new Chart(alertCtx, {
                type: 'bar',
                data: {
                    labels: [<?php foreach ($alertsBySeverity as $a)
                        echo "'" . ucfirst($a['severity']) . "',"; ?>],
                    datasets: [{
                        label: 'Alerts',
                        data: [<?php foreach ($alertsBySeverity as $a)
                            echo $a['count'] . ","; ?>],
