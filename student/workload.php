    ...
    <div class="content-wrapper">
        ...
        <div class="card mb-4">
            <div class="card-header">
                <h6><i class="fas fa-calendar-week me-2"></i>This Week's Assessment Load</h6>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <?php foreach ($dayAssessments as $day => $count):
                        ...
                        <div class="col">
                            <div class="workload-day <?= $level ?>">
                                ...
                                <small
                                    class="text-muted"><?= $count === 0 ? 'Free' : ($count === 1 ? 'assessment' : 'assessments') ?></small>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
                            ...
                            <tbody>
                                <?php foreach ($weekAssess as $wa): ?>
                                    <tr>
                                        ...
                                        <td><?= e($wa['code']) ?></td>
                                        <td><span
                                                class="badge <?= getAssessmentBadge($wa['type'])['class'] ?>"><?= ucfirst($wa['type']) ?></span>
                                        </td>
        ...
        <?php endforeach; ?>
        <?php if (empty($weeks)): ?>
            <div class="empty-state"><i class="fas fa-calendar-check"></i>
                <h5>No Upcoming Assessments</h5>
                <p>You have no assessments in the next 4 weeks.</p>
            </div>
        <?php endif; ?>
    </div>
</main>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (weekChartCtx) {
            new Chart(weekChartCtx, {
                type: 'bar',
                data: {
                    labels: [<?php foreach ($dayAssessments as $d => $c)
                        echo "'$d',"; ?>],
                    datasets: [{
                        label: 'Assessments',
