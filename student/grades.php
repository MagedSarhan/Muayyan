...
<main class="main-content">
    <div class="content-wrapper" id="reportContent">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-lg-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h6>Course Averages</h6>
                    </div>
                    <div class="card-body">
                        <?php foreach ($courseData as $cd): ?>
                            <div class="mb-3">
                                <div class="d-flex justify-content-between mb-1" style="font-size:.8rem"><span
                                        class="fw-semibold"><?= e($cd['code']) ?></span><span
                                        class="grade-cell <?= $cd['avg'] >= 80 ? 'high' : ($cd['avg'] >= 60 ? 'mid' : 'low') ?>"><?= $cd['avg'] ?>%</span>
                                </div>
                                <div class="progress" style="height:6px">
                                    <div class="progress-bar <?= $cd['avg'] >= 80 ? 'success' : ($cd['avg'] >= 60 ? 'warning' : 'danger') ?>"
                                        style="width:<?= $cd['avg'] ?>%"></div>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
        ...
        <?php foreach ($courseData as $cd):
            ...
            <div class="card mb-3">
                <div class="card-header">
                    <h6><i class="fas fa-book me-2"></i><?= e($cd['code']) ?> — <?= e($cd['name']) ?></h6><span
                        class="badge <?= $cd['avg'] >= 80 ? 'bg-success' : ($cd['avg'] >= 60 ? 'bg-warning text-dark' : 'bg-danger') ?>">Avg:
                        <?= $cd['avg'] ?>%</span>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        foreach ($allGrades as $ag) {
            $labels[] = substr($ag['title'], 0, 15);
            $values[] = round(($ag['score'] / $ag['max_score']) * 100, 1);
        }
        ?>
        ...
        if (trendCtx) {
            new Chart(trendCtx, {
                type: 'line',
                data: {
                    labels: [<?php foreach ($labels as $l)
                        echo "'" . addslashes($l) . "',"; ?>],
                    datasets: [{
                        label: 'Score %',
                        data: [<?= implode(',', $values) ?>],
