...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: ' . BASE_URL . '/advisor/notes.php');
    exit;
}
...
<main class="main-content">
    <div class="content-wrapper">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#noteModal"><i
                    class="fas fa-plus me-1"></i>Add Note</button>
        </div>
                ...
                <div class="card-body py-3">
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="d-flex gap-2 align-items-start">
                            <div class="student-avatar" style="width:32px;height:32px;font-size:.65rem">
                                <?= getInitials($n['student_name']) ?></div>
                            <div>
                                <strong style="font-size:.85rem"><?= e($n['student_name']) ?></strong>
                                <span
                                    class="badge bg-<?= $typeColors[$n['note_type']] ?? 'secondary' ?> ms-1"><?= ucfirst(str_replace('_', ' ', $n['note_type'])) ?></span>
                                <?php if ($n['is_private']): ?><span class="badge bg-dark ms-1"><i
                                            class="fas fa-lock me-1"></i>Private</span><?php endif; ?>
                                <p class="mb-0 mt-1" style="font-size:.82rem"><?= nl2br(e($n['content'])) ?></p>
                                <small class="text-muted">By <?= e($n['author_name']) ?> &bull;
                                    <?= timeAgo($n['created_at']) ?></small>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Add Academic Note</h5><button type="button" class="btn-close"
                    data-bs-dismiss="modal"></button>
            </div>
            <form method="POST">
                <div class="modal-body">
                    <div class="mb-3"><label class="form-label">Student</label><select name="student_id"
                            class="form-select" required><?php foreach ($myStudents as $s): ?>
                                <option value="<?= $s['id'] ?>"><?= e($s['name']) ?></option><?php endforeach; ?>
                        </select></div>
                    <div class="mb-3"><label class="form-label">Type</label><select name="note_type"
                            class="form-select">
                    ...
                    <div class="mb-3"><label class="form-label">Note</label><textarea name="content"
                            class="form-control" rows="4" required></textarea></div>
                    <div class="form-check"><input type="checkbox" name="is_private" class="form-check-input"
                            id="isPrivate"><label class="form-check-label" for="isPrivate">Private (only visible to
                            faculty/advisors)</label></div>
                </div>
