...
<main class="main-content">
    <div class="content-wrapper" id="reportContent">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <?php if ($studentData): ?><button class="btn btn-outline-primary" onclick="printReport('reportContent')"><i
                        class="fas fa-print me-1"></i>Print</button><?php endif; ?>
        </div>
        <div class="filter-bar mb-4">
            <form method="GET" class="d-flex gap-2"><select name="student" class="form-select"
                    onchange="this.form.submit()">
                    <option value="">Select Student</option><?php foreach ($students as $s): ?>
                        <option value="<?= $s['id'] ?>" <?= $studentId == $s['id'] ? 'selected' : '' ?>>
                            <?= e($s['name'] . ' (' . $s['user_id'] . ')') ?></option><?php endforeach; ?>
                </select></form>
        </div>
        <?php if ($studentData):
            $risk = $studentData['risk'];
            $badge = getRiskBadge($risk['level']); ?>
            <div class="row g-4">
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body text-center">
                            <div class="student-avatar mx-auto mb-3" style="width:60px;height:60px;font-size:1.2rem">
                                <?= getInitials($studentData['name']) ?></div>
                            <h5 class="fw-bold"><?= e($studentData['name']) ?></h5>
                            <p class="text-muted mb-3"><?= e($studentData['user_id']) ?> &bull;
                                <?= e($studentData['department']) ?></p>
                            <div class="risk-indicator justify-content-center">
                                <div class="risk-circle <?= $risk['level'] ?>"><?= round($risk['score']) ?></div>
                                <div class="risk-info text-start">
                                    <h5><?= $badge['label'] ?></h5>
                                    <p>Avg: <?= $risk['avg_grade'] ?>% | <?= ucfirst($risk['trend']) ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-8">
                    <div class="card h-100">
                        <div class="card-header">
                            <h6>Performance by Course</h6>
                        </div>
                        <div class="card-body">
                            <div style="height:250px"><canvas id="courseChart"></canvas></div>
                        </div>
                    </div>
                </div>
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h6>Detailed Grade Breakdown</h6>
                        </div>
        ...
        <?php else: ?>
            <div class="empty-state"><i class="fas fa-user-graduate"></i>
                ...
                <p>Choose a student to view their detailed performance report.</p>
            </div>
        <?php endif; ?>
    </div>
</main>
