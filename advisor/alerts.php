...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['resolve_id'])) {
    ...
    setFlash('success', 'Alert resolved.');
    header('Location: ' . BASE_URL . '/advisor/alerts.php');
    exit;
}
...
<main class="main-content">
    <div class="content-wrapper">
        ...
        <?php foreach ($alerts as $al):
            ...
            <div class="alert-card severity-<?= $al['severity'] ?> <?= $al['is_resolved'] ? 'opacity-50' : '' ?>"
                style="border:1px solid var(--gray-200)">
                <div class="d-flex justify-content-between align-items-start">
                    <div class="d-flex gap-3 align-items-start">
                        <div class="student-avatar" style="width:36px;height:36px;font-size:.7rem">
                            <?= getInitials($al['student_name']) ?></div>
                        <div>
                            <div class="alert-title"><?= e($al['title']) ?></div>
                            <div class="alert-message"><?= e($al['message']) ?></div>
                            <div class="alert-time mt-1">
                                <span class="me-3"><i class="fas fa-user me-1"></i><?= e($al['student_name']) ?>
                                    (<?= e($al['stu_uid']) ?>)</span>
                                <?php if ($al['course_code']): ?><span class="me-3"><i
                                            class="fas fa-book me-1"></i><?= e($al['course_code']) ?></span><?php endif; ?>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <?php if (!$al['is_resolved']): ?>
                            <form method="POST"><input type="hidden" name="resolve_id" value="<?= $al['id'] ?>"><button
                                    class="btn btn-sm btn-outline-success" title="Resolve"><i class="fas fa-check"></i></button>
                            </form>
                        <?php else: ?>
