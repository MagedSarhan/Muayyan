<?php
/** Muayyan - Advisor Contact Requests */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'reply') {
        ...
        $req->execute([$_POST['request_id']]); $r = $req->fetch();
    ...
    header('Location: ' . BASE_URL . '/advisor/requests.php'); exit;
}
    ...
    <div class="card mb-3">
        <div class="card-header">
            <div>
                ...
                <small class="text-muted">From: <?= e($req['student_name']) ?> (<?= e($req['stu_uid']) ?>) &bull; <?= timeAgo($req['created_at']) ?></small>
            </div>
        ...
        <div class="card-body">
            ...
            <div class="d-flex gap-2 mb-2 p-2 rounded" style="background:<?= $rp['role']==='advisor' ? 'var(--primary-25)' : 'var(--gray-50)' ?>">
                <div class="student-avatar" style="width:28px;height:28px;font-size:.6rem"><?= getInitials($rp['name']) ?></div>
                <div><strong style="font-size:.78rem"><?= e($rp['name']) ?></strong> <small class="text-muted">(<?= ucfirst($rp['role']) ?>) &bull; <?= timeAgo($rp['created_at']) ?></small><p class="mb-0" style="font-size:.82rem"><?= nl2br(e($rp['message'])) ?></p></div>
            </div>
            <?php endforeach; ?>
            <?php endif; ?>
            ...
            <form method="POST" class="d-flex gap-2 align-items-end">
                <input type="hidden" name="action" value="reply"><input type="hidden" name="request_id" value="<?= $req['id'] ?>">
                <div class="flex-fill"><textarea name="message" class="form-control" rows="2" placeholder="Write your reply..." required></textarea></div>
                <div class="d-flex flex-column gap-1">
                    <button type="submit" class="btn btn-primary btn-sm"><i class="fas fa-reply me-1"></i>Reply</button>
                </div>
            </form>
            <form method="POST" class="mt-2"><input type="hidden" name="action" value="status"><input type="hidden" name="request_id" value="<?= $req['id'] ?>">
