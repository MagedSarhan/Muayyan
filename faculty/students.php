<?php
/** MOEEN  - Faculty Student Performance */
...
<main class="main-content">
    <div class="content-wrapper">
        ...
        <div class="filter-bar mb-4">
            <form method="GET" class="d-flex gap-2"><select name="section" class="form-select"
                    onchange="this.form.submit()">
                    <option value="">Select Section</option><?php foreach ($mySections as $ms): ?>
                        ...
                        <tbody>
                            <?php foreach ($students as $st):
                                ...
                                <tr>
                                    ...
                                    <td><span
                                            class="badge <?= $risk['trend'] === 'improving' ? 'bg-success' : ($risk['trend'] === 'declining' ? 'bg-danger' : 'bg-secondary') ?>"><i
                                                class="fas fa-<?= $risk['trend'] === 'improving' ? 'arrow-up' : ($risk['trend'] === 'declining' ? 'arrow-down' : 'minus') ?> me-1"></i><?= ucfirst($risk['trend']) ?></span>
                                    </td>
                                    <td><span class="risk-badge risk-<?= $risk['level'] ?>"><i
                                                class="fas <?= $badge['icon'] ?> me-1"></i><?= $badge['label'] ?></span></td>
                                    <td><button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                            data-bs-target="#studentDetail<?= $st['id'] ?>"><i
                                                class="fas fa-eye me-1"></i>Details</button></td>
                                </tr>
                                <!-- Student Detail Modal -->
                                <div class="modal fade" id="studentDetail<?= $st['id'] ?>" tabindex="-1">
                                    <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title"><?= e($st['name']) ?> — Performance Details</h5><button
                                                    type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <div class="risk-indicator mb-3">
                                                    <div class="risk-circle <?= $risk['level'] ?>"><?= round($risk['score']) ?>
                                                    </div>
                                                    <div class="risk-info">
                                                        <h5><?= $badge['label'] ?></h5>
                                                        <p>Risk Score: <?= $risk['score'] ?>/100 | Avg:
                                                            <?= $risk['avg_grade'] ?>%</p>
                                                    </div>
                                                </div>
        ...
        <?php elseif (!$sectionId): ?>
            <div class="empty-state"><i class="fas fa-hand-point-up"></i>
                ...
                <p>Choose a section to view student performance.</p>
            </div>
        <?php endif; ?>
    </div>
</main>
