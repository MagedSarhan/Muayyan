...
$params = [$fid];
if ($sectionFilter) { $where .= " AND a.section_id = ?"; $params[] = $sectionFilter; }
...
<div class="content-wrapper">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#assessModal"><i class="fas fa-plus me-1"></i> New Assessment</button>
    </div>
    <div class="filter-bar mb-3">
        <form method="GET" class="d-flex gap-2">
            <select name="section" class="form-select" onchange="this.form.submit()">
                ...
                <option value="<?= $ms['id'] ?>" <?= $sectionFilter==$ms['id']?'selected':'' ?>><?= e($ms['code'].' - '.$ms['name'].' (Sec '.$ms['section_number'].')') ?></option>
                <?php endforeach; ?>
            </select>
        </form>
    </div>
    <div class="table-container"><div class="table-responsive"><table class="table">
        <thead><tr><th>Assessment</th><th>Course</th><th>Type</th><th>Max Score</th><th>Weight</th><th>Due Date</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="<?= BASE_URL ?>/faculty/grades.php?section=<?= $a['section_id'] ?>&assessment=<?= $a['id'] ?>" class="btn btn-sm btn-outline-primary" title="Grade"><i class="fas fa-pen"></i></a>
                <form method="POST" class="d-inline" onsubmit="return confirmDelete()"><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= $a['id'] ?>"><button class="btn btn-sm btn-outline-danger"><i class="fas fa-trash"></i></button></form>
            </td>
        </tr>
        <?php endforeach; ?>
        </tbody>
...
<div class="modal fade" id="assessModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <form method="POST"><div class="modal-body"><input type="hidden" name="action" value="add">
        <div class="mb-3"><label class="form-label">Section</label><select name="section_id" class="form-select" required><?php foreach($mySections as $ms): ?><option value="<?= $ms['id'] ?>" <?= $sectionFilter==$ms['id']?'selected':'' ?>><?= e($ms['code'].' - '.$ms['name']) ?></option><?php endforeach; ?></select></div>
        <div class="mb-3"><label class="form-label">Title</label><input type="text" name="title" class="form-control" required></div>
        <div class="row g-3 mb-3">
            <div class="col-4"><label class="form-label">Type</label><select name="type" class="form-select"><option>quiz</option><option>midterm</option><option>final</option><option>project</option><option>assignment</option><option>presentation</option><option>lab</option><option>participation</option></select></div>
            <div class="col-4"><label class="form-label">Max Score</label><input type="number" name="max_score" class="form-control" value="10" step="0.5" required></div>
            <div class="col-4"><label class="form-label">Weight %</label><input type="number" name="weight" class="form-control" value="5" step="0.5" required></div>
        </div>
    ...
    </div><div class="modal-footer"><button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary">Create</button></div></form>
</div></div></div>
