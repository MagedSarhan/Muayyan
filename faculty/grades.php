...
if ($currentAssessment) { $currentAssessment->execute([$assessmentId]); $currentAssessment = $currentAssessment->fetch(); }
        ...
        <form method="GET" class="d-flex gap-3 flex-wrap w-100">
            <div>
                ...
                <select name="section" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="<?= $ms['id'] ?>" <?= $sectionId==$ms['id']?'selected':'' ?>><?= e($ms['code'].' - '.$ms['name'].' (Sec '.$ms['section_number'].')') ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="assessment" class="form-select" onchange="this.form.submit()">
                    ...
                    <option value="<?= $a['id'] ?>" <?= $assessmentId==$a['id']?'selected':'' ?>><?= e($a['title']) ?> (<?= ucfirst($a['type']) ?> - <?= $a['max_score'] ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            <?php endif; ?>
    ...
    <div class="card">
        <div class="card-header">
            <div>
                ...
                <small class="text-muted">Max Score: <?= $currentAssessment['max_score'] ?> | Weight: <?= $currentAssessment['weight_percentage'] ?>% | Due: <?= formatDate($currentAssessment['due_date']) ?></small>
            </div>
        </div>
        <div class="card-body">
            <form method="POST">
                ...
                <div class="table-responsive">
                    <table class="table">
                        <thead><tr><th>#</th><th>Student</th><th>ID</th><th>Score (Max: <?= $currentAssessment['max_score'] ?>)</th><th>Status</th></tr></thead>
                        <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="number" name="grades[<?= $st['id'] ?>]" class="form-control" style="max-width:120px" step="0.5" min="0" max="<?= $currentAssessment['max_score'] ?>" value="<?= e($existing) ?>" placeholder="—"></td>
                            <td>
                                <?php if ($pct >= 0): ?>
                                <span class="grade-cell <?= $pct >= 80 ? 'high' : ($pct >= 60 ? 'mid' : 'low') ?>"><?= round($pct) ?>%</span>
                                <?php else: ?>
                ...
                <button type="submit" class="btn btn-primary mt-3"><i class="fas fa-save me-1"></i> Save Grades</button>
            </form>
        </div>
    </div>
    <?php elseif (!$sectionId): ?>
    <div class="empty-state"><i class="fas fa-hand-point-up"></i><h5>Select a Section</h5><p>Choose a section and assessment to start entering grades.</p></div>
    <?php elseif (!$assessmentId): ?>
    <div class="empty-state"><i class="fas fa-clipboard-list"></i><h5>Select an Assessment</h5><p>Choose an assessment to enter grades for.</p></div>
    <?php endif; ?>
</div>
</main>
