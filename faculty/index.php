...
<div class="content-wrapper">
    ...
    <div class="row g-3 mb-4">
        <div class="col-xl-3 col-md-6"><div class="stat-card"><div class="stat-icon primary"><i class="fas fa-chalkboard"></i></div><div class="stat-value" data-count="<?= $mySections ?>"><?= $mySections ?></div><div class="stat-label">My Sections</div></div></div>
        <div class="col-xl-3 col-md-6"><div class="stat-card"><div class="stat-icon success"><i class="fas fa-user-graduate"></i></div><div class="stat-value" data-count="<?= $myStudents ?>"><?= $myStudents ?></div><div class="stat-label">Total Students</div></div></div>
        <div class="col-xl-3 col-md-6"><div class="stat-card"><div class="stat-icon warning"><i class="fas fa-clipboard-list"></i></div><div class="stat-value" data-count="<?= $myAssessments ?>"><?= $myAssessments ?></div><div class="stat-label">Active Assessments</div></div></div>
        <div class="col-xl-3 col-md-6"><div class="stat-card"><div class="stat-icon danger"><i class="fas fa-pen"></i></div><div class="stat-value" data-count="<?= $pendingGrades ?>"><?= $pendingGrades ?></div><div class="stat-label">Pending Grade Entry</div></div></div>
    </div>
    ...
    <div class="row g-4">
                ...
                <div class="col-md-6">
                    <div class="course-card">
                        ...
                        <div class="mt-3 d-flex gap-2">
                            ...
                            <a href="<?= BASE_URL ?>/faculty/students.php?section=<?= $s['id'] ?>" class="btn btn-sm btn-outline-primary flex-fill"><i class="fas fa-users me-1"></i>Students</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
        ...
        <div class="col-lg-4">
            ...
            <?php if (empty($atRisk)): ?>
            <div class="empty-state p-3"><i class="fas fa-check-circle text-success" style="font-size:2rem"></i><p class="mt-2 mb-0 text-success fw-semibold">All students performing well!</p></div>
            <?php else: ?>
            <?php foreach ($atRisk as $ar): ?>
            <div class="alert-card severity-<?= $ar['severity'] ?> mb-2">
                <div class="d-flex align-items-center gap-2 mb-1">
                    <div class="student-avatar" style="width:28px;height:28px;font-size:.6rem"><?= getInitials($ar['name']) ?></div>
            ...
            <div class="timeline-item">
                <div class="timeline-icon <?= strtotime($ua['due_date']) < strtotime('+3 days') ? 'danger' : 'primary' ?>"><i class="fas <?= getAssessmentBadge($ua['type'])['icon'] ?>"></i></div>
                <div class="timeline-content">
                    <h6><?= e($ua['title']) ?></h6>
                    <p><?= e($ua['course_code']) ?> &bull; <span class="badge <?= getAssessmentBadge($ua['type'])['class'] ?>"><?= ucfirst($ua['type']) ?></span></p>
                    <span class="time"><i class="fas fa-calendar-day me-1"></i><?= formatDate($ua['due_date']) ?></span>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
